...
const Simulator = function (wgl, shaderSources, resolutionWidth, resolutionHeight) {
  this.wgl = wgl;
  ...
  this.splatProgram = wgl.createProgram(
      shaderSources['splat.vert'], shaderSources['splat.frag']);

  this.velocitySplatProgram = wgl.createProgram(
      '#define VELOCITY \n' + shaderSources['splat.vert'], '#define VELOCITY \n' + shaderSources['splat.frag']);

  this.advectProgram = wgl.createProgram(
      shaderSources['fullscreen.vert'], shaderSources['advect.frag']);

  this.divergenceProgram = wgl.createProgram(
      shaderSources['fullscreen.vert'], shaderSources['divergence.frag']);

  this.jacobiProgram = wgl.createProgram(
      shaderSources['fullscreen.vert'], shaderSources['jacobi.frag']);

  this.subtractProgram = wgl.createProgram(
      shaderSources['fullscreen.vert'], shaderSources['subtract.frag']);

  this.copyProgram = wgl.createProgram(
      shaderSources['fullscreen.vert'], shaderSources['output.frag']);

  this.resizeProgram = wgl.createProgram(
      shaderSources['fullscreen.vert'], shaderSources['resize.frag']);
...
Simulator.prototype.copyTexture = function (destinationWidth, destinationHeight, sourceTexture, destinationTexture) {
  var copyDrawState = this.wgl.createDrawState()
      .bindFramebuffer(this.simulationFramebuffer)
      .viewport(0, 0, destinationWidth, destinationHeight)
      .useProgram(this.copyProgram)

      .uniformTexture('u_input', 0, this.wgl.TEXTURE_2D, sourceTexture)

      .vertexAttribPointer(this.quadVertexBuffer, this.copyProgram.getAttribLocation('a_position'), 2, this.wgl.FLOAT, false, 0, 0);

  this.wgl.framebufferTexture2D(this.simulationFramebuffer, this.wgl.FRAMEBUFFER, this.wgl.COLOR_ATTACHMENT0, this.wgl.TEXTURE_2D, destinationTexture, 0);
  this.wgl.drawArrays(copyDrawState, this.wgl.TRIANGLE_STRIP, 0, 4);
};
